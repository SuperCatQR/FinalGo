...
> 关联:CHO-106(轨道 A 内容填埋)。本文件锁定「🟡 机器初稿 → 🟢 可发布」的机器闸门与人工前置条件,供 SME 校对、法务合规、QA 提测三方对齐。
...
3. **校对签名可追溯**(CHK-12):`reviewed: true` + `reviewer` 须由真实人工校对者在对应 Git commit 署名,**不得由自动化 agent 伪造**。
...
- ⚠️ 真题 2025-04 / 2025-10 缺失:`source/past-papers/` 仅有 README,无任何 A 级官方真题原件;考频/真题分布表为空态保留(符合 issue 空态规则:不删区块、不虚构)。
- ⚠️ 适用专业清单仅覆盖 1 个专业(源数据限制):15044 实为全省公共必修,待全量专业计划简编补齐后脚本更新。

## 四、交付路径

1. 后端整合 Agent:锁契约(本文件)+ 预置 15044 机器侧 frontmatter 待签字段(不伪造签名)→ 推分支 + draft PR 回链本 issue。
2. SME:逐项签名高风险区块,填实测考频/ISBN,把 `reviewed: true` + `reviewer` 写入 frontmatter。
3. 法务合规团队:版权/PII/AI 声明审视通过。
4. QA:提测,跑 `build-course-pages.py` + `check-source-links.py` 全绿后置 🟢。
5. 按 [[squad-handoff]] assign 推动流转。
